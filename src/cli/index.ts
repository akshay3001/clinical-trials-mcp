...
program
  .name('clinicaltrials')
  .description('CLI tool for searching and analyzing clinical trials from ClinicalTrials.gov')
  .version('1.0.0');
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const nctIds = studies.map(s => s.protocolSection.identificationModule.nctId);
      ...
      console.log(`Use "clinicaltrials export --session ${sessionId}" to export data`);
    } catch (error) {
